...
class SimpleTerminalController:
    def __init__(self,
                 verbatim: bool = True,
                 maxmin_velocity: float = 4, # increased
    ...
    def handle_gesture(self, gesture, quad_vel):
        ...
        current_x_vel = quad_vel.x_val
        current_y_vel = quad_vel.x_val

        # velocity = 0 if gesture = stop
        if gesture == 'Stop':
            self.vx = 0
            self.vy = 0
            self.yaw =0
            ...
            # handle changes in x-axis. If neither front nor back gesture detected, smoothly lower x velocity by *0.5
            if gesture == 'Front':
                self.vx = round(number=float(np.clip(current_x_vel + 1, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)
                ...
                self.vx = round(number=float(np.clip(current_x_vel * 0.5, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)

            # handle changes in y-axis. If neither right nor left gesture detected, smoothly lower y velocity by *0.5
            if gesture == 'Right':
                self.vy = round(number=float(np.clip(current_y_vel + 1, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)
            elif gesture == 'Left':
                self.vy = round(number=float(np.clip(current_y_vel - 1, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)
            else:
                self.vy = round(number=float(np.clip(current_y_vel * 0.5, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)


            if gesture == 'Anticlockwise':
                self.yaw = -20
            elif gesture == 'Clockwise':
                self.yaw = 20
            else:
                self.yaw = round(self.yaw * 0.5, ndigits=0)
    ...
    def smooth_decay(self, quad_vel):
        z = self.client.getMultirotorState().kinematics_estimated.position.z_val
        self.vx = round(number=float(np.clip(quad_vel.x_val * 0.5, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)
        self.vy = round(number=float(np.clip(quad_vel.y_val * 0.5, - self.maxmin_vel, self.maxmin_vel)), ndigits=2)
        self.client.moveByVelocityZAsync(self.vx, self.vy, z, 0.1, airsim.DrivetrainType.MaxDegreeOfFreedom,
                                         airsim.YawMode(True, self.yaw)).join()
